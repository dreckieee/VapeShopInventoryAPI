...
## Status: Deployed
Product CRUD, Expense CRUD, full Sale/SaleItem lifecycle (create, add/reduce items, close, cancel), monthly filtering on Sales/Expenses, a computed income endpoint, and a batch restock endpoint with itemized delivery cost tracking (`DeliveryItem`) are complete and deployed. `PaymentMethod`/`PaymentNote` tracking on Sales and Expenses is live in production as of Day 133 — enum values serialize as strings (e.g. `"Cash"`) in all API responses and accept either string or int form on input. The Settlement feature (recording payments against outstanding Receivable/Payable balances) is feature-complete, fully tested, and **deployed to production as of Day 141**: create/get endpoints, all guards (target validation, closed-sale requirement, overpayment prevention, settlement date-validity), computed `AmountSettled`/`OutstandingBalance`/`TotalAmount` fields on Sale and Expense responses, and the PaymentMethod lock guard (once any settlement exists against a record, its PaymentMethod becomes permanently uneditable) are all implemented and under direct automated test coverage, including the guard's success paths (same-value edit and no-settlement-exists cases, for both Sale and Expense). A `GET /api/Settlements` list endpoint (filterable by `?year=`/`?month=`) was added Day 142. Day 139 found and fixed two real bugs surfaced while writing that coverage: `DeleteExpense` was returning a raw `500` instead of a `409 Conflict` when a Settlement referenced the expense, and `UpdateExpense` was allowing an expense's `Amount` to be edited below its already-settled total, which could produce a nonsensical negative `OutstandingBalance`. Day 140 found and fixed a third: `CreateSettlement` had no guard preventing a settlement from being dated earlier than the Sale/Expense it settles — fixed by rejecting any settlement date earlier than the target's own date (same-day is allowed). A matching guard was added to `UpdateExpense`: once a settlement exists against an expense, its `Date` can no longer be edited to a point later than the earliest existing settlement, which would retroactively predate that settlement. Day 141 found and fixed a fourth (a test-infrastructure bug, not an API bug): several Settlement-creating tests across three test files weren't tracking the settlement records they created for teardown, leaking rows across test runs — fixed by adding proper tracking and cleanup everywhere settlements are created. A `CapitalTransaction` entity (tracking owner deposits/withdrawals separately from Expense, as a prerequisite for the upcoming Cash Balance feature) was added Day 142 with functional create/get/list endpoints, and received full automated test coverage on Day 143. 90 automated tests via `WebApplicationFactory` against an isolated in-memory database cover Product, Sale/SaleItem, Restock, Expense, Settlement, and CapitalTransaction flows, plus manual verification against the live deployed instance. Live on a DigitalOcean droplet as of Day 94, most recently redeployed Day 141.

**Not yet deployed to production:** `CapitalTransaction` (Day 142, fully tested as of Day 143 — no longer blocked by missing coverage, deploy is a candidate for the next session). A `CashBalanceCalculator` computing four running balances (Cash on Hand, Digital Balance, Receivables Outstanding, Payables Outstanding) is being built (Day 143, formula written but not yet verified or committed) as a prerequisite for an upcoming Cash Balance endpoint — no endpoint exists yet, so nothing is API-visible from this work.
...
### Capital Transactions *(fully tested — not yet deployed)*
