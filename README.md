...
- `Product.Id` has a private setter and is not a constructor parameter (correct for domain construction — a new product has no id until the database assigns one), which means `System.Text.Json` cannot populate it by default when deserializing an API response. Fixed by adding `[JsonInclude]` on the `Id` property — this only affects serialization visibility, it does not add a public setter or change how `Id` can be set from application code. This gap exists because `Product` currently doubles as both the domain entity and the API request/response contract (the controller accepts and returns `Product` directly, with no dedicated DTO) — a future move to a dedicated Products DTO layer (mirroring what Sales already has) would remove the need for this attribute
...
## Day 87 — Status
Build 3 remains fully complete. Added a third API-mode Playwright test: `CreateProduct_ValidProduct_ReturnsCreated`, covering `POST /api/Products` — asserts `201 Created` and validates every returned field against the sent payload. Introduced a `[TearDown]` method (`DeleteTestProduct`) that automatically deletes any product created during a test, using a nullable class-level field (`_createdProductId`) set only on successful creation, so GET-only tests safely no-op. During implementation, found and fixed a real bug: `Product.Id` couldn't be populated on deserialization (private setter, not a constructor parameter), silently leaving it at `0` and causing teardown to call `DELETE` on the wrong id — fixed by adding `[JsonInclude]` to `Id`. Deployment remains on hold pending outcome of the family card conversation; will check back next session. Remaining before Step 2 is done:
- Deploy the Web API (roadmap's Step 2 target) — pending family card conversation outcome; Azure B1s Southeast Asia selected as primary path, Hetzner CX22 as fallback
- Next planned test: a POST/failure-case test (e.g. invalid payload), or introducing `[SetUp]`/shared base class now that request-context boilerplate is duplicated across all four tests
